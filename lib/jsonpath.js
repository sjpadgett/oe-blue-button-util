...
var jsonPathMethods = {
    ...
    traceProperties: function (properties, index, obj, path) {
        ...
        if (properties) {
            properties.forEach(function (p) {
                ...
                var v = this.traceProperty(p, index, obj, newPath);
                append(result, v);
            }, this);
        }
    ...
    traceProperty: function (property, index, obj, path) {
        ...
        if (obj && obj.hasOwnProperty(property)) {
            path.push(property);
            var v = this.traceNext(index, obj[property], path);
            append(result, v);
        }
        return this.transferResult(obj, result);
    },
    traceAll: function (index, obj, path) {
        ...
        if (ps) {
            ps.forEach(function (p) {
                var newPath = path.slice();
                newPath.push(p);
                var v = this.traceAll(index, obj[p], newPath);
                append(result, v);
            }, this);
        }
        return this.transferResult(obj, result);
...
var exprToJsonPathMethod = {
    ...
    objFunction: function (fn, expr) {
        return function (index, obj, path) {
            var result = [];
            path.push(expr);
            var v = this.traceNext(index, fn(obj), path);
            append(result, v);
            return this.transferResult(obj, result);
        };
    }
