...
var jsonPathMethods = {
    traceAllProperties: function (index, jspResult) {
        var obj = jspResult.currentObject();
        var ps = properties(obj);
        return this.traceProperties(ps, index, jspResult);
    },
    traceProperties: function (properties, index, jspResult) {
        if (properties) {
            properties.forEach(function (p) {
                var newJspResult = jspResult.clone();
                this.traceProperty(p, index, newJspResult);
            }, this);
        }
    },
    traceProperty: function (property, index, jspResult) {
        var obj = jspResult.currentObject();
        if (obj && obj.hasOwnProperty(property)) {
            jspResult.add(obj[property], property);
            this.traceNext(index, jspResult);
        }
    },
    traceAll: function (index, jspResult) {
        var rootJspResult = jspResult.clone();
        this.traceNext(index, rootJspResult);
        var obj = jspResult.currentObject();
        var ps = properties(obj);
        if (ps) {
        ...
        if (ps) {
            ps.forEach(function (p) {
                ...
                this.traceAll(index, newJspResult);
            }, this);
        }
    },
    traceBack: function (index, jspResult) {
        jspResult.back();
        this.traceNext(index, jspResult);
    },
    traceEnd: function (index, jspResult) {
        jspResult.addToResult();
    },
    traceNext: function (index, jspResult) {
        ++index;
        var action = this.actions[index];
        action.call(this, index, jspResult);
    },
...
var exprToJsonPathMethod = {
    jsExprProperty: function (expr) {
        return function (index, jspResult) {
            var path = jspResult.pathHistory;
            var obj = jspResult.currentObject();
            var property = this.eval(expr, obj, path[path.length], jspResult);
            this.traceProperty(property, index, jspResult);
        };
    },
    filteredProperties: function (expr) {
        ...
        return function (index, jspResult) {
            ...
            this.traceProperties(ps, index, jspResult);
        };
    },
    commaDelimitedProperties: function (expr) {
        var properties = expr.split(',');
        return function (index, jspResult) {
            this.traceProperties(properties, index, jspResult);
        };
    },
    arrayRange: function (expr) {
        ...
        return function (index, jspResult) {
            ...
            this.traceProperties(range, index, jspResult);
        };
    },
...
exports.instance = function (inputExpr, opts) {
    ...
    return function (obj) {
        ...
        jspResult.init(obj, '$');
        jsonPath.traceNext(-1, jspResult);
