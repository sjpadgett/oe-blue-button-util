...
exports.instance = function (inputExpr, opts) {
    var exprToFn = function (expr) {
        if (expr === '*') {
            ...
            return jsonPathMethods.expressionProperties(expr);
        } else if (expr.indexOf('?(') === 0) { // [?(expr)]
            return jsonPathMethods.filteredProperties(expr);
        } else if (expr.indexOf(',') > -1) { // [name1,name2,...]
            return jsonPathMethods.listProperties(expr);
        } else if (/^(-?[0-9]*):(-?[0-9]*):?([0-9]*)$/.test(expr)) { // [start:end:step]  Python slice syntax
            return jsonPathMethods.indexProperties(expr);
        } else if (expr === '^') {
            return jsonPathMethods.toParent;
        } else if (expr === '$') {
            return expr;
        } else {
            return jsonPathMethods.tracePropertyFn(expr);
        }
    };
