...
var jsonPathMethods = {
    traceStart: function (obj, actions) {
        var result = this.traceNext(actions, obj, ['$']);
        return result;
    },
    traceAllProperties: function (actions, obj, path) {
    ...
    traceProperty: function (property, actions, obj, path) {
        ...
        if (obj && obj.hasOwnProperty(property)) {
            var v = this.traceNext(actions, obj[property], push(path, property));
            append(result, v);
        }
        return this.transferResult(obj, result);
    },
    traceAll: function (actions, obj, path) {
        var result = [];
        var rootResult = this.traceNext(actions, obj, path);
        ...
        if (ps) {
            ps.forEach(function (p) {
                var v = this.traceAll(actions, obj[p], push(path, p));
                append(result, v);
            }, this);
        }
        return this.transferResult(obj, result);
    },
    traceBack: function (actions, obj, path) {
        return path.length ? [{
            path: path.slice(0, -1),
            expr: actions,
    ...
    traceEnd: function (actions, obj, path) {
        return [{
            path: path,
            value: obj
        }];
    },
    traceNext: function (actions, obj, path) {
        var action = actions[0];
        var remainingActions = actions.slice(1);

        return action.call(this, remainingActions, obj, path);
    },
    ...
    transferResult: function (obj, result) {
        ...
        return result.reduce(function (all, ea) {
            if (ea.isParentSelector) {
                var v = self.traceNext(ea.expr, obj, ea.path);
                return all.concat(v);
            } else {
                all.push(ea);
                return all;
...
var exprToJsonPathMethod = {
    ...
    objFunction: function (fn, expr) {
        return function (actions, obj, path) {
            ...
            var v = this.traceNext(actions, fn(obj), newPath);
            append(result, v);
            return this.transferResult(obj, result);
        };
    }
...
exports.instance = function (inputExpr, opts) {
    ...
    return function (obj) {
        ...
        var ret = jsonPath.traceNext(exprList, obj, ['$']);
